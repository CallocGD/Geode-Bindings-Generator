use crate::{
    parsing_logic::{is_static_func, should_comment_out_function},
    re,
};
...
pub trait SignatureResults {
    ...
    fn group_for_function<'c, 'f>(&self, class_name: &'c str, func_sig: &'f str) -> Group {
        ...
        match self.vtable_index_for_func(&class_name, &func_sig) {
            Some(vi) => {
                return Group::VTINDEX(vi);
            }
            None => return Group::INDEX(-2),
        }
    }

    fn reorder_funcs<'c, 'f>(
        &self,
        class_name: &'c str,
        funcs: &'f Vec<String>,
    ) -> BTreeMap<i32, Vec<HashSet<String>>> {
        /*
        ...
        for (k, v) in first {
            ...
            for func_pos in v {
                inner_dict
                    .entry(func_pos.1)
                    .and_modify(|v| v.push(func_pos.0.clone()))
                    .or_insert(vec![func_pos.0]);
            }
            ...
            for mut values in inner_dict {
                ...
                new_funcs
                    .entry(k)
                    .and_modify(|x| x.push(sorted_values.clone()))
                    .or_insert(vec![sorted_values]);
            }
        }
        println!(
            "DEBUGGING LOGIC FOR REORDER FUNCS {class_name} -> {:?}",
            new_funcs
        );
        return new_funcs;
    }
...
impl SignatureResults for OldSignatureResults {
    ...
    fn best_effort_guess<'c, 'n>(&self, class_name: &'c str, name: &'n str) -> String {
        ...
        if (name.starts_with("ccTouchBegan(")
            || name.starts_with("ccTouchMoved(")
            || name.starts_with("ccTouchEnded(")
            || name.starts_with("ccTouchCancelled("))
            && name.contains("(cocos2d::CCTouch*, cocos2d::CCEvent*)")
        {
            return format!(
                "void {}(cocos2d::CCTouch* pTouch, cocos2d::CCEvent* pEvent)",
                name.split_once('(').unwrap().0,
            );
        }

        if name.starts_with("create(")
            || (name.starts_with("shared") && is_static_func(class_name, &name))
        {
        ...
        if re::GET_ATTR.is_match(&name) {
            ...
            for func in cls_table {
                if func.starts_with(&set_name) {
                    return regex_replace!(r#"set[A-Z]\w+\(([^\,\)]+)\)"#, func, |_, v| format!(
                        "{v} {name}"
                    ))
                    .to_string();
                }
    ...
    fn add_func_to_class<'c, 'f>(&mut self, class_name: &'c str, func_sig: &'f str) {
        self.classes
            ...
            .and_modify(|c| c.push(func_sig.to_string()))
            .or_insert(vec![func_sig.to_string()]);
    }
