...
pub fn do_old_version(
    ...
    println!(
        "Loading Android symbols from \"{}\"",
        android_symbols_file.display()
    );
    ...
    unsafe {
        for capture in android_symbols
            .iter()
        ...
        {
            let groups: Vec<&str> = capture
                .iter()
                .filter(|c| c.is_some())
                .map(|c| c.unwrap_unchecked().as_str())
                .collect();
    ...
    let virtuals: HashMap<String, serde_json::Value> =
        pl::read_vtables_json_file(&vtables.unwrap());
    let mut virtual_tables: HashMap<String, Vec<Vec<String>>> = HashMap::new();
    for (k, v) in &virtuals {
        let vals: Vec<Vec<String>> = v
            .as_array()
            .unwrap()
            .iter()
            .map(|x| {
    ...
    {
        let tables = virtuals[name]
            .as_array()
            .expect("Ghidra configured your Vtables Wrong!");
        // println!("VTABLE {name} -> {:?}", tables);
        let pure_virts: HashSet<String> = tables
            .iter()
            .map(|x| x.as_array().unwrap().iter().map(|x| x.as_str().unwrap()))
            .flatten()
            .filter(|x| x.starts_with("pure_virtual_"))
            .map(String::from)
            .collect();
    ...
    osr.write()
        .save(&std::path::PathBuf::from("out/Geometrydash.bro"));
    println!("Finished");
}
